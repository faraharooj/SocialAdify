// D:/socialadify/frontend/src/app/ad-creator/layout.tsx
'use client';
...
const AdPlacementIcon = ({ className = "w-8 h-8" }) => {
...
const NavLink: React.FC<{ href: string; children: ReactNode }> = ({ href, children }) => {
    ...
    // Logic to highlight parent routes
    const isActive = (href === "/home" && pathname === "/home") || 
...
// Renamed component to AdCreatorLayout
export default function AdCreatorLayout({ children }: { children: ReactNode; }) {
            ...
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-600 via-slate-800 to-slate-900 text-slate-100">
                    ...
                    <div className="w-full p-0">
                        <div className="flex items-center justify-between h-10 md:h-18 px-4 sm:px-6 lg:px-8">
                               <div className="flex items-center gap-2.5 select-none cursor-default">
                                    ...
                                    <AdPlacementIcon className="h-8 w-8 sm:h-9 sm:w-9 transition-transform duration-300 group-hover:rotate-[7deg] group-hover:scale-105" />
                                    <span className="text-xl sm:text-2xl font-bold text-slate-50 group-hover:text-sky-400 transition-colors duration-300 tracking-tight">
                                        Ad Placement
                                    </span>
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        {/* Updated footer text */}
                        © {new Date().getFullYear()} SocialAdify. AI Ad Placement Module.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
