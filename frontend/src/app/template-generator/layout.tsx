...
// New Icon for Template Generator
const TemplateHeaderIcon = ({ className = "w-6 h-6" }: { className?: string }) => ( <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9.53 16.122a3 3 0 00-5.78 1.128 2.25 2.25 0 01-2.4 2.245 4.5 4.5 0 008.4-2.245c0-.399-.078-.78-.22-1.128zm0 0a15.998 15.998 0 003.388-1.62m-5.043-.025a15.994 15.994 0 011.622-3.395m3.42 3.42a15.995 15.995 0 004.764-4.648l3.876-5.814a1.151 1.151 0 00-1.597-1.597L14.146 6.32a15.996 15.996 0 00-4.649 4.763m3.42 3.42a6.776 6.776 0 00-3.42-3.42" /></svg> );
...
export default function TemplateGeneratorLayout({
    children,
}: {
    children: ReactNode;
}) {
            ...
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-800 to-slate-900 text-slate-100">
                    ...
                    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-10 md:h-18">
                            ...
                            <div className="flex items-center">
                                <div className="flex items-center">
                                    {/* --- UPDATED: Changed text-pink-500 to text-sky-400 to match Visual Ad Generator --- */}
                                    <TemplateHeaderIcon className="w-6 h-6 sm:w-8 sm:h-8 mr-3 text-sky-400" />
                                    ...
                                    >
                                        {(profilePicUrl && !profileImageError) ? (
                                            <Image 
                                                ...
                                                width={40} 
                                                height={40}
                                                // --- UPDATED: ring-pink-500 -> ring-sky-400 ---
                                                className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-sky-400 transition-all"
                                                onError={() => setProfileImageError(true)}
                                            />
                                        ) : (
                                            // --- UPDATED: text-pink-500 -> text-sky-400 ---
                                            <UserIcon className="h-8 w-8 text-slate-300 group-hover:text-sky-400 transition-colors" /> 
                                        )}
                                        {user.firstname && (
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        © {new Date().getFullYear()} SocialAdify. Template Studio.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
