// D:\socialadify\frontend\src\app\scheduler\layout.tsx
'use client';
...
const PostSchedulingIcon = ({ className = "w-8 h-8" }) => {
...
export default function SchedulerLayout({ children }: { children: ReactNode; }) {
    ...
    return (
        <ProtectedRoute>
            {/* --- MODIFIED: Refined background gradient --- */}
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-600 via-slate-800 to-slate-900 text-slate-100">
                    ...
                    <div className="w-full p-0">
                        <div className="flex items-center justify-between h-10 md:h-18 px-4 sm:px-6 lg:px-8">
                               <div className="flex items-center gap-2.5 select-none cursor-default">
                                    ...
                                    <PostSchedulingIcon className="h-8 w-8 sm:h-9 sm:w-9 transition-transform duration-300 group-hover:rotate-[7deg] group-hover:scale-105" />
                                    ...
                                    >
                                        {(profilePicUrl && !profileImageError) ? (
                                            <Image
                                                ...
                                                width={40}
                                                height={40}
                                                className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-orange-400 transition-all"
                                                onError={() => setProfileImageError(true)}
                                            />
                                        ) : (
                                            <UserIcon className="h-8 w-8 text-slate-300 group-hover:text-orange-400 transition-colors" />
                                        )}
                                        {user.firstname && (
                        ...
                        {/* --- MODIFIED: Added full navigation for mobile --- */}
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        © {new Date().getFullYear()} SocialAdify. Post Scheduling Module.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
