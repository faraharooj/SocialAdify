// D:\socialadify\frontend\src\app\social-insights\layout.tsx
'use client';
...
// --- Icon Components (from your reference) ---
...
const SocialInsightsIcon = ({ className = "w-8 h-8" }) => {
...
// Use the environment variable for static assets
const API_BASE_URL_STATIC = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';

// --- Navigation Links (with "Social Insights" added) ---
const navLinks = [
...
export default function SocialInsightsLayout({ children }: { children: ReactNode; }) {
            ...
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-600 via-slate-800 to-slate-900 text-slate-100">
                    ...
                    <div className="w-full p-0">
                        <div className="flex items-center justify-between h-10 md:h-18 px-4 sm:px-6 lg:px-8">
                               <div className="flex items-center gap-2.5 select-none cursor-default">
                                    ...
                                    <SocialInsightsIcon className="h-8 w-8 sm:h-9 sm:w-9 transition-transform duration-300 group-hover:rotate-[7deg] group-hover:scale-105" />
                                    <span className="text-xl sm:text-2xl font-bold text-slate-50 group-hover:text-orange-400 transition-colors duration-300 tracking-tight">
                                        Social Insights
                                    </span>
                                    ...
                                    >
                                        {(profilePicUrl && !profileImageError) ? (
                                            <Image
                                                ...
                                                width={40}
                                                height={40}
                                                className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-indigo-400 transition-all"
                                                onError={() => setProfileImageError(true)}
                                            />
                                        ) : (
                                            <UserIcon className="h-8 w-8 text-slate-300 group-hover:text-indigo-400 transition-colors" />
                                        )}
                                        {user.firstname && (
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        © {new Date().getFullYear()} SocialAdify. Social Media Insights Module.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
