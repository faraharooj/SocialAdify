...
// Using a Clock/History icon for the header title
const HistoryHeaderIcon = ({ className = "w-6 h-6" }: { className?: string }) => ( <svg className={className} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> );
    ...
    const profilePicUrl = 
        user?.profile_picture_url && user.profile_picture_url.trim() !== '' 
            ? (user.profile_picture_url.startsWith('http') 
                ...
                : `${API_BASE_URL_STATIC}/${user.profile_picture_url.replace('/static/', 'static/')}`)
            : null;

    return (
        <ProtectedRoute>
            {/* Kept your original background gradient as requested */}
            <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-gray-800 to-slate-900 text-slate-100">
                {/* Header structure matching Connections Layout */}
                <header className="bg-slate-900/80 backdrop-blur-md shadow-xl sticky top-0 z-50 border-b border-slate-700/50">
                    <div className="w-full px-4 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between h-10 md:h-18">
                            
                            {/* Left Side: Page Title (Matched to Connections style) */}
                            <div className="flex items-center">
                                <div className="flex items-center">
                                    <HistoryHeaderIcon className="w-6 h-6 sm:w-8 sm:h-8 mr-3 text-purple-400" />
                                    ...
                                    >
                                        {(profilePicUrl && !profileImageError) ? (
                                            <Image 
                                                ...
                                                width={40} 
                                                height={40}
                                                className="rounded-full object-cover ring-1 ring-slate-600 group-hover:ring-purple-400 transition-all"
                                                onError={() => setProfileImageError(true)}
                                            />
                                        ) : (
                                            <UserIcon className="h-8 w-8 text-slate-300 group-hover:text-purple-400 transition-colors" /> 
                                        )}
                                        {user.firstname && (
                ...
                <footer className="bg-slate-900/60 border-t border-slate-700/60 text-center py-6 mt-auto">
                    <p className="text-xs text-slate-500">
                        © {new Date().getFullYear()} SocialAdify. Your Content History.
                    </p>
                </footer>
            </div>
        </ProtectedRoute>
